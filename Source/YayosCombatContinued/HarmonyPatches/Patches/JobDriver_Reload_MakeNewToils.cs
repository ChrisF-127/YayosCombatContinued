...
namespace YayosCombatContinued
{
	...
	{
		// see HarmonyPatches for SimpleSidearms compatibility Prefix

		private static IEnumerable<Toil> Postfix(IEnumerable<Toil> values, JobDriver_Reload __instance, Pawn ___pawn, Job ___job)
		{
			...
			var apparelComp = gear?.TryGetComp<CompApparelReloadable>();
			var abilityComp = gear?.TryGetComp<CompEquippableAbilityReloadable>();
			...
			var reloadables = new List<IReloadableComp>
			{
				apparelComp,
				abilityComp,
			};
			...
			toil.initAction = () =>
			{
				if (!___pawn.carryTracker.CarriedThing.Destroyed)
		...
		private static IEnumerable<Toil> ReloadAsMuchAsPossible(List<IReloadableComp> reloadables, Pawn pawn)
		{
			...
			yield return Toils_Jump.JumpIf(done, () =>
			{
				if (pawn.carryTracker.CarriedThing == null)
					return true;
				return pawn.carryTracker.CarriedThing.stackCount < reloadables.First(comp => comp.AmmoDef == pawn.carryTracker.CarriedThing.def).MinAmmoNeeded(true);
			});
